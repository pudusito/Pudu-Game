...
//funcion que nos permite dibujar en el canvas(Carga del canvas y dibujo de la nave)
...
//funcion para recargar el frame de la imagen(*********GAMELOOP*******)
function update() {
    requestAnimationFrame(update);
...
alienArray = alienArray.filter(alien => alien.alive);
    ...
    if (alienCount == 0) {
        if (nivel > 0){
        score += alienColumnas * alienFilas * 1; //bonus point al subir de nivel :).  //incrementa el numero de enemigos en columnas y filas en 1
